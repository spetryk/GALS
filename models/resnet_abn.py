...
# NOTE: I believe these model_urls are for default resnet models, not ABN versions.
# See repo linked at top for pretrained ABN weights.
model_urls = {
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, add_after_attention=False):
        self.inplanes = 64
        super(ResNet, self).__init__()
        self.add_after_attention = add_after_attention
    ...
    def forward(self, x, provided_att=None):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        if provided_att is None:
            ...
            att = transforms.functional.resize(
                provided_att, x.shape[2:]
            )
            self.att = att
            ax = None
        ...
        # self.att: B x    1 x 14 x 14
        # ax:       B x num_classes
        rx = x * self.att # shape: B x 1024 x 14 x 14
        if self.add_after_attention:
            rx = rx + x       # shape: B x 1024 x 14 x 14
        per = rx
        rx = self.layer4(rx)  # shape: B x 2048 x 7 x 7
        rx = self.avgpool(rx) # shape: B x 2048 x 1 x 1
        rx = rx.view(rx.size(0), -1)
        rx = self.fc(rx) # shape: B x num_classes

        # fe:  B x 1024 x 14 x 14
        # per: B x 1024 x 14 x 14
